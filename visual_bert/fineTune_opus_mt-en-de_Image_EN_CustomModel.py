...
class BestModels:
    ...
    def __call__(self, score, model, translation_model, tokenizer, epoch):
        if len(self.best_models) < self.k:
            heapq.heappush(self.best_models, (score, epoch, model, translation_model, tokenizer))
            self.save_checkpoint(score, model, translation_model, tokenizer, epoch)
        else:
            if score > self.best_models[0][0]:  # Compare with the smallest score in the heap
                _, _, old_model, old_translation_model, old_tokenizer = heapq.heappop(self.best_models)
                heapq.heappush(self.best_models, (score, epoch, model, translation_model, tokenizer))
                self.save_checkpoint(score, model, translation_model, tokenizer, epoch)

    def save_checkpoint(self, score, translation_model, model, tokenizer, epoch):
        """Saves model when validation score improves."""
        path = f"best_model_epoch_{epoch}_score_{score:.4f}"
        if self.verbose:
            print(f"Validation score improved. Saving model to {path}...")
        translation_model.save_pretrained(path)
...
def train_translation_model(model, clip_processor, text_model, text_tokenizer, train_dataloader, valid_dataloader,
    ...
    for epoch in range(num_epochs):
        ...
        best_models(bleu_score.score, model, model.translation_model, model.translation_tokenizer, epoch)
...
num_epochs=30
warmup_epochs = 8
num_warmup_steps = (warmup_epochs / num_epochs) * num_training_steps
#scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=num_training_steps // 3, gamma=0.1) #original
...
train_translation_model(model, clip_processor, text_model, text_tokenizer, train_dataloader, valid_dataloader,
                        optimizer, scheduler, device, generation_config, num_epochs=num_epochs)
